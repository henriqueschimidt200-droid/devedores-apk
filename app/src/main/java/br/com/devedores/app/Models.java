...
public class Models {
    public static class Client {
        ...
        public String name = "", cpf = "", rg = "", phone = "", address = "", notes = "";
        ...
        public JSONObject toJson() {
            try {
                ...
                o.put("phone", phone); o.put("address", address); o.put("notes", notes); o.put("createdAt", createdAt);
        ...
        public static Client fromJson(JSONObject o) {
            ...
            try {
                ...
                c.phone = o.optString("phone", ""); c.address = o.optString("address", ""); c.notes = o.optString("notes", "");
                ...
                // Compatibilidade com versões antigas que salvavam apenas URI em docs[]
    ...
    public static class Loan {
        ...
        public String frequency="Mensal"; public int installments=1; public long firstDue; public long createdAt=System.currentTimeMillis();
        ...
        public JSONObject toJson(){try{JSONObject o=new JSONObject();o.put("id",id);o.put("title",title);o.put("notes",notes);o.put("principal",principal);o.put("interestPercent",interestPercent);o.put("interestValue",interestValue);o.put("total",total);o.put("installmentAmount",installmentAmount);o.put("frequency",frequency);o.put("installments",installments);o.put("firstDue",firstDue);o.put("createdAt",createdAt);JSONArray p=new JSONArray();for(Payment x:payments)p.put(x.toJson());o.put("payments",p);return o;}catch(Exception e){return new JSONObject();}}
        public static Loan fromJson(JSONObject o){Loan l=new Loan();try{l.id=o.optString("id",l.id);l.title=o.optString("title","Empréstimo");l.notes=o.optString("notes","");l.principal=o.optDouble("principal");l.interestPercent=o.optDouble("interestPercent");l.interestValue=o.optDouble("interestValue");l.total=o.optDouble("total");l.installmentAmount=o.optDouble("installmentAmount");l.frequency=o.optString("frequency","Mensal");l.installments=o.optInt("installments",1);l.firstDue=o.optLong("firstDue");l.createdAt=o.optLong("createdAt",l.createdAt);JSONArray a=o.optJSONArray("payments");if(a!=null)for(int i=0;i<a.length();i++)l.payments.add(Payment.fromJson(a.getJSONObject(i)));}catch(Exception ignored){}return l;}
    }
    ...
    public static class Reminder {public String id=UUID.randomUUID().toString(), title="Lembrete", details="";public long when;public boolean done;public JSONObject toJson(){try{JSONObject o=new JSONObject();o.put("id",id);o.put("title",title);o.put("details",details);o.put("when",when);o.put("done",done);return o;}catch(Exception e){return new JSONObject();}}static Reminder fromJson(JSONObject o){Reminder r=new Reminder();r.id=o.optString("id",r.id);r.title=o.optString("title","Lembrete");r.details=o.optString("details");r.when=o.optLong("when");r.done=o.optBoolean("done",false);return r;}}
}
